...
def search_arr(arr, term):
    for a in arr:
        aux_dict = {k.lower():v for k, v in a.items()}
        aux_term = term.lower()
        if aux_term in aux_dict:
            return aux_dict.get(aux_term)


def get_aws_service_actions(service_name, action_name):
    ...
    service = search_arr(actions, action_url)
    return service
...
def main():
    ...
    print('************* Searching write actions on policy *************')
    for pa in policy_actions:
        pac = pa.split(':')
        service_name = pac[0]
        action_name = pac[1]
        service = get_aws_service_actions(service_name, action_name)
        if service['actionGroups'][0] == 'ReadWrite':
            print(f'write permission found in service: {service_name}, action {action_name}')
